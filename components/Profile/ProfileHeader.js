...
function ProfileHeader({ profile, ownAccount, loggedUserFollowStats, setLoggedUserFollowStats }) {
	...
	const isFollowing =
		loggedUserFollowStats.following.length > 0 && loggedUserFollowStats.following.filter((following) => following.user === profile.user._id).length > 0;
					...
					<Grid.Column
						...
						{!ownAccount && (
							<Button
								...
								onClick={async () => {
									setLoading(true);
									isFollowing ? await unfollowUser(profile.user._id, setLoggedUserFollowStats) : await followUser(profile.user._id, setLoggedUserFollowStats);
									setLoading(false);
								}}
							/>
						)}
